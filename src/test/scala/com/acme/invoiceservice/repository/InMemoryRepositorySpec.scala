  ...
  "InMemoryRepository" - {
    "get all" - {
      "should fetch all items after adding items" in {
        val invoices = List(
          ...
          Invoice("3", "customer3", "address3", 3, "regular", "invoice", dateTime, dateTime, 3, dateTime, dateTime, "description", 10, 10, 10)
        )
    ...
    "query" - {
         ...
         val invoices =  List(
            ...
            Invoice("10", "customer10", "address10", 10, "regular", "invoice", dateTime, dateTime, 10, dateTime, dateTime, "description", 10, 10, 10)
          )

      "should apply AndFilter correctly" in {
        invoices.foreach(inMemoryRepository.add)
        val filterResult: List[Invoice] = inMemoryRepository.query(AndFilter(List(MatchFilter("invoiceType", "shop"), MatchFilter("customerId", "customer1"))))
      ...
      "should apply OrFilter correctly" in {
        invoices.foreach(inMemoryRepository.add)
        val filterResult: List[Invoice] = inMemoryRepository.query(OrFilter(List(MatchFilter("invoiceType", "shop"), MatchFilter("customerId", "customer1"))))
        assert(filterResult.size == 2)
        inMemoryRepository.clear()
      }
      "should apply NotFilter correctly" in {
        invoices.foreach(inMemoryRepository.add)
        val filterResult: List[Invoice] = inMemoryRepository.query(NotFilter(AndFilter(List(MatchFilter("invoiceType", "shop"), MatchFilter("customerId", "customer1")))))
        assert(filterResult.size == 9)
        inMemoryRepository.clear()
      }
      "should apply MatchFilter correctly" in {
        invoices.foreach(inMemoryRepository.add)
        val filterResult: List[Invoice] = inMemoryRepository.query(MatchFilter("invoiceType", "shop"))
        assert(filterResult.size == 2)
        inMemoryRepository.clear()
